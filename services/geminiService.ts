export const analyzePlantImage = async (imageBuffer: string) => {
  ...
  const prompt = "Analise a planta e retorne APENAS um JSON com: scientificName, commonName, orixRuling, fundamento, fundamentoExplanation, eweClassification, ritualNature, applicationLocation (lista), stepByStepInstructions (lista), prayer (objeto title/text), goldenTips (objeto title/content), elements, historicalContext, safetyWarnings, suggestedTitles.";
